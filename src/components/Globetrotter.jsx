...
export default function GlobetrotterGame() {
  ...
  const getAllQuestions = async () => {
    const fetchData = await fetch(`${process.env.REACT_APP_BACKEND_URL}/questions`);
  ...
  const fetchScore = async () => {
    const response = await fetch(
      `${process.env.REACT_APP_BACKEND_URL}/score?username=${username}`
    );
    const data = await response.json();
    setScore(data);
  };
  ...
  const getUserByReferralID = async (referralCode) => {
    const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/getUserById`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        referralCode,
  ...
  const handleSubmit = async () => {
    ...
    const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/submit-answer`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        username,
